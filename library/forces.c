...
void physics_collision_handler(body_t *body1, body_t *body2, vector_t axis,
  ...
  if (aux != NULL) {
    elasticity = *(double *)aux;
  }

  double impulse = 0.0;
  double ua = vec_dot(vel1, axis);
  double ub = vec_dot(vel2, axis);
  if (mass1 == INFINITY) {
    impulse = (-mass2 * (1 + elasticity) * (ub - ua));
  } else if (mass2 == INFINITY) {
    impulse = (mass1 * (1 + elasticity) * (ub - ua));
  } else {
    impulse =
        ((mass1 * mass2) / (mass1 + mass2)) * (1 + elasticity) * (ub - ua);
  }
